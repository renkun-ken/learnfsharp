...
module lineChart =
    ...
    let expectedIncome =
        [ for x in 1..100 -> (futureDate x, 1000.0 + rand() * 100.0 * exp(float x / 40.0)) ]

    let expectedExpenses =
        [ for x in 1..100 -> (futureDate x, rand() * 500.0 * sin(float x / 50.0)) ]
...
module Stock =
    ...
    let goog =
        load "GOOG" 80
        |> Array.map(fun tick -> (tick.Date.ToShortDateString(), tick.High, tick.Low, tick.Open, tick.Close))
        |> Chart.Candlestick
...
module boxPlot =
    ...
    Chart.BoxPlotFromData
        (threeSyntheticDataSets, ShowUnusualValues = true, ShowMedian = true, ShowAverage = true, WhiskerPercentile = 10)