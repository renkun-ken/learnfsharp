...
let readData2 list = 
    list |> List.iter(function 
                | x when x >= 0 -> printfn "%d" x
                | x when x <= -3 -> 
                    raise(System.ArgumentException("It's wrong", "x"))
...
let tryError1 = 
    ...
    with :? System.ArgumentException as ex -> 
        printfn "Incorrect argument: %s" ex.ParamName

// try..finally
let divide x y = 
    let stream: System.IO.FileStream = 
        System.IO.File.Create(@"d:\data\test\test.txt")
