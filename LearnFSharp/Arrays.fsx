...
let arrayFind = 
    ...
    let element = 
        arrayA |> Array.find(fun x -> isPerfectSquare x && isPerfectCube x)
    let index = 
        arrayA |> Array.findIndex(fun x -> isPerfectSquare x && isPerfectCube x)
    (index, element)
...
let arrayTryFind = 
    ...
    let look array1 = 
        let result = 
            array1 
            |> Array.tryFind(fun x -> isPerfectSquare x && isPerfectCube x)
        match result with
...
let arrayPick = 
    ...
    let test x = 
        if isPerfectSquare x && isPerfectCube x then 
            Some(x, intFun sqrt x, intFun cubeRoot x)
        else None
    
    let pick array1 = 
        match Array.tryPick test array1 with
        | Some(n, sqrt, cuberoot) -> 
            printfn "%d, sqrt=%d, cuberoot=%d" n sqrt cuberoot
